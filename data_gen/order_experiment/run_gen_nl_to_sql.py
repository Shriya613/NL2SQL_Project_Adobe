...
SQL_GEN_PROMPT = """
    ...
    creative, and non-trivial SQLite query that answers a natural language question.
    ...
    8. Answer the natural language question: {nl_question}
...
NL_GEN_PROMPT = """
    You are a natural language expert tasked with generating a natural language question.
    Given the table information, generate an interesting and complex natural language question.

    Your natural language question should:
    1. Only produce a question. Do not include the table in 
    your response.
    2. Be specific enough that it can be answered by a single SQLite query.
    3. Not include details that the table information does not contain.
...
def generate_sql_query(table: str, nl_question: str) -> str:
    """Generate SQL query based on table"""
    prompt_formatted = SQL_GEN_PROMPT.format(table=table, nl_question=nl_question)
...
def correct_sql_query(table: str, sql_query: str, nl_question: str, error: str) -> str:
    prompt_formatted = SQL_CORRECT_PROMPT.format(sql_query=sql_query, nl_question=nl_question, error=error, table=table)
    return prompt_formatted

def generate_nl(table: str) -> str:
    prompt_formatted = NL_GEN_PROMPT.format(table=table)
    return prompt_formatted
...
def run_pipeline():
    ...
    with open("data/train.tables.jsonl", "r") as f:
        ...
        for table in tqdm(table_data[:1000]):
            ...
            # Generate NL
            nl_prompt = generate_nl(table)
            nl_ques = run_llm(nl_prompt)
            cleaned_nl = clean_output(nl_ques, "question")


            sql_prompt = generate_sql_query(table, cleaned_nl)
            ...
            while db_ans == None and tries < 3:
                tries += 1
                sql_prompt = correct_sql_query(table, cleaned_sql, cleaned_nl, error)
                sql_query = run_llm(sql_prompt)
                cleaned_sql = clean_output(sql_query, "sql")
                run_sql_query, db_ans, error = filter_sql_query(table, cleaned_sql, cur)
            ...
            if db_ans != None:
                ...
                # SQL query that was run on the database
                actual_sql.append(run_sql_query)

                generated_nl.append(cleaned_nl)
                generated_sql.append(cleaned_sql)
        
        pd.DataFrame({"table_info": table_info, "nl": generated_nl, "sql": generated_sql, "actual_sql": actual_sql, "ans": database_ans}).to_csv("data_gen/order_experiment/generated_nl_sql.csv", index=False)

def evaluate_pipeline():
    calculate_complexity("data_gen/order_experiment/generated_nl_sql.csv", "actual_sql", "nl")
