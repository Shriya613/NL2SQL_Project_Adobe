"""
CodeBERT-based regression model with sigmoid head for reward modeling.

This is a CodeBERT version of the BERT reward model that uses
a CodeBERT encoder with a sigmoid head to directly predict similarity scores.
"""
...
class RewardDataset(Dataset):
    """Dataset for CodeBERT reward model training."""
...
class CodeBERTRewardModel(torch.nn.Module):
    """CodeBERT model with sigmoid head for reward prediction."""
    
    def __init__(self, model_name='microsoft/codebert-base'):
        super().__init__()
        self.codebert = AutoModel.from_pretrained(model_name)
        self.dropout = torch.nn.Dropout(0.1)
        self.classifier = torch.nn.Linear(self.codebert.config.hidden_size, 1)
        self.sigmoid = torch.nn.Sigmoid()
        
    def forward(self, input_ids, attention_mask, labels=None):
        outputs = self.codebert(input_ids=input_ids, attention_mask=attention_mask)
        
        # CodeBERT (based on RoBERTa) doesn't have pooler_output, so use mean pooling
        # Get the last hidden state
        last_hidden_state = outputs.last_hidden_state
        
        # Mean pooling: sum over sequence length, weighted by attention mask
        ...
        if labels is not None:
            loss_fct = torch.nn.MSELoss()  # MSELoss (Mean Squared Error)
...
def main():
    ...
    output_dir = 'outputs/codebert_reward_model_mse_loss'
    batch_size = 16  
    num_epochs = 30
    learning_rate = 2e-5
    ...
    print(f"📏 Filtering examples over 512 tokens (CodeBERT's limit)...")
    ...
    # Set max_length to 512 (CodeBERT's maximum, same as BERT)
    max_length = 512
    print(f"\n📏 Using max_length: {max_length} tokens (CodeBERT's maximum)")
    ...
    train_file = os.path.join(output_dir, 'train_set_mse_loss.csv')
    eval_file = os.path.join(output_dir, 'eval_set_mse_loss.csv')
    test_file = os.path.join(output_dir, 'test_set_mse_loss.csv')
    ...
    model = CodeBERTRewardModel(model_name)
    ...
    # Train
    print("🚀 Starting CodeBERT reward model training...")
    trainer.train()
    ...
    results_file = os.path.join(output_dir, 'training_results_mse_loss.json')
